...
export const ObrasView = {
    ...
    renderDashboard: (obra, stats) => {
        ...
        return `
            ...
            content: `<p id="kpi-total-orcado" class="text-3xl font-display text-text">${Utils.formatCurrency(stats.totalPlanned || stats.financialSummary?.total?.planned || 0)}</p>`
        })}
                    ${Components.createCard({
            title: 'Total Gasto (Mat + M.O.)',
            content: `<p id="kpi-total-gasto" class="text-3xl font-display text-text">${Utils.formatCurrency(stats.totalSpent || stats.financialSummary?.total?.spent || stats.totalGasto || 0)}</p><p class="text-xs heading-muted mt-1" id="kpi-total-saldo-label">Saldo: ${Utils.formatCurrency(stats.totalBalance || stats.financialSummary?.total?.balance || 0)}</p>`
        })}
                    ${Components.createCard({
            title: '% Gasto Total',
            content: `<p id="kpi-total-percent" class="text-3xl font-display text-${(stats.totalPercent || 0) > 100 ? 'alert' : 'primary'}">${(stats.totalPercent || 0).toFixed(1)}%</p>`
        })}
                    ${Components.createCard({
            ...
            content: `<p class="text-4xl font-display text-primary uppercase">${stats.aguardandoEntrega || 0}</p><p class="text-sm heading-muted mt-1">Em trânsito</p>`
        })}
                    ${Components.createCard({
            title: 'SLA Entregas',
            ...
            content: `<p class="text-4xl font-display text-text uppercase">${(stats.lead || 0).toFixed(1)}d</p><p class="text-sm heading-muted mt-1">Emissão → Entrega/Previsão</p>`
                ...
                content: `<p class="text-3xl font-display text-primary uppercase">${(stats.horasPrevistasEq || horasOrcadasTotais || 0).toFixed(1)}h</p><p class="text-xs heading-muted mt-1">Normal: ${stats.horasNormaisPrevistas || 0}h • Extras: ${stats.horasExtrasPrevistas || 0}h</p>`,
                className: 'accent-left'
            })}
                        ${Components.createCard({
                title: 'Horas Executadas (Eq.)',
                content: `<p class="text-3xl font-display text-text uppercase">${(stats.horasExecutadasEq || (stats.rdoData?.totalHoras || 0)).toFixed(1)}h</p><p class="text-xs heading-muted mt-1">Normal: ${(stats.horasNormaisExecutadas || 0).toFixed(1)}h • Extras: ${(stats.horasExtrasExecutadas || 0).toFixed(1)}h</p>`
            })}
                        ${Components.createCard({
                title: 'Saldo de Horas',
                content: `<p id="kpi-rdo-saldo" class="text-3xl font-display text-${(stats.saldoHorasEq || 0) < 0 ? 'alert' : 'primary'} uppercase">${(stats.saldoHorasEq || 0).toFixed(1)}h</p><p id="kpi-rdo-saldo-sub" class="text-xs heading-muted mt-1">~${(stats.rdoData?.saldoDias || 0).toFixed(1)} dias</p>`
            })}
                        ${Components.createCard({
                title: 'Custo Estimado M.O.',
                content: `<p class="text-3xl font-display text-text uppercase">${Utils.formatCurrency(stats.laborSpent || 0)}</p><p class="text-xs heading-muted mt-1">Horas × R$/h</p>`
            })}
                        ${Components.createCard({
                title: 'Horas Extras Totais',
                content: `<p id="kpi-rdo-extras" class="text-3xl font-display text-alert uppercase">${(stats.horasExtrasExecutadas || stats.rdoData?.totalExtras || 0).toFixed(1)}h</p><p id="kpi-rdo-extras-sub" class="text-xs heading-muted mt-1">Previstas: ${(stats.horasExtrasPrevistas || horasExtrasPlanejadas).toFixed(1)}h</p>`,
                className: 'border-l-4 border-alert'
            })}
                        ${Components.createCard({
                title: '% Extras / Normais',
                content: `<p class="text-3xl font-display text-${(stats.percentExtrasNormais || 0) > 20 ? 'alert' : 'text'} uppercase">${(stats.percentExtrasNormais || 0).toFixed(1)}%</p><p class="text-xs heading-muted mt-1">Índice de extras</p>`
            })}
                    </div>
