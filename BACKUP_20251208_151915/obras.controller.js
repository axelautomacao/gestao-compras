...
export const ObrasController = {
    ...
    initForm: async (obraId = null) => {
        ...
        try {
            let obra = null;
            const users = await SettingsService.getUsers().catch(() => []);
            if (obraId) {
                obra = await ObrasService.getObraById(obraId);
                if (obra) obra._users = users;
            } else {
                obra = { _users: users };
            }
    ...
    initDashboard: async (obraId) => {
        ...
        try {
            ...
            const stats = await ObrasService.getObraStats(obraId, true);
            const orcamento = Number(obra.valor_orcado || 0);
            stats.horasNormaisPrevistas = Number(obra.horas_previstas || 0);
            stats.horasExtrasPrev = Number(obra.horas_extras_previstas || 0);
            stats.horasExtrasPrevistas = Number(obra.horas_extras_previstas || 0);
            if (orcamento > 0) {
            ...
            const updateFinancialKpis = () => {
                ...
                const elDiff = document.getElementById('kpi-total-balance');
                ...
                if (elDiff) {
                    elDiff.textContent = Utils.formatCurrency(total.balance || 0);
                    elDiff.classList.remove('text-alert', 'text-primary');
                    elDiff.classList.add((total.balance || 0) < 0 ? 'text-alert' : 'text-primary');
                }
            };
            updateFinancialKpis();
            ...
            const renderCharts = () => {
                ...
                if (stats._evmCurve) {
                    EVMCharts.renderEVMDashboard('chart-evm-dashboard', stats._evmCurve);
                }
            };
            ...
            setTimeout(async () => {
                    ...
                    if (osNumber) {
                        ...
                        if (rdo && rdo.reports) {
                            ...
                            if (processed) {
                                ...
                                // Recalcular métricas de horas com o RDO completo e atualizar os cards
                                const rdoMetricsFull = RDOService.calculateLaborMetrics(obra, processed);
                                if (rdoMetricsFull) {
                                    stats.rdoMetrics = rdoMetricsFull;
                                    stats.horasNormaisPrevistas = rdoMetricsFull.horasNormaisPrevistas;
                                    stats.horasExtrasPrevistas = rdoMetricsFull.horasExtrasPrevistas;
                                    stats.horasPrevistasEq = rdoMetricsFull.horasPrevistasEq;
                                    stats.horasNormaisExecutadas = rdoMetricsFull.horasNormaisExecutadas;
                                    stats.horasExtrasExecutadas = rdoMetricsFull.horasExtrasExecutadas;
                                    stats.horasExecutadasEq = rdoMetricsFull.horasExecutadasEq;
                                    stats.saldoHorasEq = rdoMetricsFull.saldoHorasEq;
                                    stats.percentExtrasNormais = rdoMetricsFull.percentExtrasNormais;
                                    stats.laborSpent = rdoMetricsFull.custoEstimado ?? stats.laborSpent;
                                }

                                const extraFactor = Number(EXTRA_FACTOR || 1.5);
                                setKpi('kpi-horas-previstas-eq', `${(stats.horasPrevistasEq || 0).toFixed(1)}h`);
                                setKpi('kpi-horas-previstas-det', `Normal: ${(Number(stats.horasNormaisPrevistas) || 0).toFixed(1)}h | Extras: ${(Number(stats.horasExtrasPrevistas) || 0).toFixed(1)}h`);
                                setKpi('kpi-horas-executadas-eq', `${(stats.horasExecutadasEq || 0).toFixed(1)}h`);
                                setKpi('kpi-horas-executadas-det', `Normal: ${(Number(stats.horasNormaisExecutadas) || 0).toFixed(1)}h | Extras (${extraFactor}x): ${((Number(stats.horasExtrasExecutadas) || 0) * extraFactor).toFixed(1)}h`);
                                setKpi('kpi-horas-saldo-eq', `${(stats.saldoHorasEq || 0).toFixed(1)}h`);
                                const saldoEqEl = document.getElementById('kpi-horas-saldo-eq');
                                if (saldoEqEl) {
                                    saldoEqEl.className = `text-3xl font-display text-${(stats.saldoHorasEq || 0) < 0 ? 'alert' : 'primary'} uppercase`;
                                }
                                const saldoEqSub = document.getElementById('kpi-horas-saldo-eq-sub');
                                if (saldoEqSub) saldoEqSub.textContent = 'Horas disponiveis';
                                setKpi('kpi-horas-extras-total', `${(Number(stats.horasExtrasExecutadas) || 0).toFixed(1)}h`);
                                setKpi('kpi-horas-extras-prev', `Previstas: ${(Number(stats.horasExtrasPrevistas) || 0).toFixed(1)}h`);
                                setKpi('kpi-percent-extras', `${(Number(stats.percentExtrasNormais) || 0).toFixed(1)}%`);
                                const percentExtrasEl = document.getElementById('kpi-percent-extras');
                                if (percentExtrasEl) {
                                    percentExtrasEl.className = `text-3xl font-display text-${(Number(stats.percentExtrasNormais) || 0) > 20 ? 'alert' : 'text'} uppercase`;
                                }
                                ...
                                if (processed.totalHoras > 0) {
                                    ...
                                    if (start && end && !Number.isNaN(start) && !Number.isNaN(end) && start <= end && horasTotalPlanejadas > 0) {
                                        const workdays = [];
                                        const cursor = new Date(start); // iniciar exatamente na data cadastrada
                                        cursor.setHours(12, 0, 0, 0);
                                        const endShifted = new Date(end);
                                        endShifted.setDate(endShifted.getDate() + 1); // manter fim +1 dia
                ...
                try {
                    ...
                    // Métricas de mão de obra (Sprint 2)
                    const rdoM = RDOService.calculateLaborMetrics(obra, stats.rdoData || {});
                    stats.rdoMetrics = rdoM;
                    stats.horasNormaisPrevistas = Number(obra.horas_previstas || 0);
                    stats.horasExtrasPrevistas = Number(obra.horas_extras_previstas || 0);
                    stats.horasPrevistasEq = stats.horasNormaisPrevistas + (stats.horasExtrasPrevistas * 1.5);

                    let horasNormaisExec = Number(rdoM.horasNormaisExecutadas ?? stats.horasNormaisExecutadas ?? 0);
                    let horasExtrasExec = Number(rdoM.horasExtrasExecutadas ?? stats.horasExtrasExecutadas ?? 0);
                    if ((!horasNormaisExec && !horasExtrasExec) && (stats.rdoData?.totalHoras || stats.rdoData?.totalExtras)) {
                        const totalExtras = Number(stats.rdoData?.totalExtras || 0);
                        const totalHoras = Number(stats.rdoData?.totalHoras || 0);
                        horasExtrasExec = totalExtras;
                        horasNormaisExec = Math.max(0, totalHoras - totalExtras);
                    }
                    stats.horasNormaisExecutadas = horasNormaisExec;
                    stats.horasExtrasExecutadas = horasExtrasExec;
                    stats.horasExecutadasEq = rdoM.horasExecutadasEq ?? (stats.horasNormaisExecutadas + stats.horasExtrasExecutadas * 1.5);
                    stats.saldoHorasEq = rdoM.saldoHorasEq ?? (stats.horasPrevistasEq - stats.horasExecutadasEq);
                    const custoMO = (horasNormaisExec * COST_PER_HOUR) + (horasExtrasExec * COST_PER_OVERTIME_HOUR);
                    stats.laborSpent = Number(rdoM.custoEstimado ?? stats.laborSpent ?? custoMO);
                    stats.percentExtrasNormais = Number(
                        rdoM.percentExtrasNormais ??
                        (stats.horasNormaisExecutadas > 0 ? (stats.horasExtrasExecutadas / stats.horasNormaisExecutadas) * 100 : 0)
                    );

                    // EVM (Sprint 3)
                    stats._evmCurve = EVMService.generateEVMCurve(obra, stats.comprasCalendar || stats.comprasRecentes || [], stats.rdoData || null);
                    stats.evmData = stats._evmCurve?.evm;
                } catch (err) {
    ...
    bindFormEvents: (obraId) => {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                data.responsavel_id = formData.get('responsavel_id') || null;
