    ...
    renderDiretor: (stats) => {
        return `
            ...
            content: `<p class="text-3xl font-display text-text uppercase">${stats.alerts?.cotacao || 0}</p><p class="text-sm heading-muted mt-1">Ped. em cotação</p>`
                ...
                <div class="space-y-2 mt-6">
                    <h3 class="text-lg font-display text-text uppercase tracking-wide border-b border-border pb-2">
                        ƒð? Indicadores de Qualidade
                    </h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                        ${Components.createCard({
            title: 'Taxa de Retrabalho',
            content: `<p class="text-4xl font-display text-${stats.reworkRate?.status === 'good' ? 'primary' : stats.reworkRate?.status === 'warning' ? 'amber-500' : 'alert'} uppercase">
                                    ${(stats.reworkRate?.rate || 0).toFixed(1)}%
                                 </p>
                                 <p class="text-sm heading-muted mt-1">
                                    ${stats.reworkRate?.canceladas || 0} canceladas/devolvidas
                                 </p>
                                 <p class="text-xs heading-muted mt-1">Meta: < 5% | Status: </p>`,
            className: 'accent-left'
        })}
                        ${Components.createCard({
            title: 'Índice de Conformidade',
            content: `<p class="text-4xl font-display text-${stats.complianceIndex?.status === 'good' ? 'primary' : stats.complianceIndex?.status === 'warning' ? 'amber-500' : 'alert'} uppercase">
                                    ${(stats.complianceIndex?.index || 0).toFixed(1)}%
                                 </p>
                                 <p class="text-sm heading-muted mt-1">
                                    ${stats.complianceIndex?.compliant || 0} com docs completos
                                 </p>
                                 <p class="text-xs heading-muted mt-1">Meta: > 90% | Faltando: </p>`
        })}
                        ${Components.createCard({
            ...
            content: `<p class="text-3xl font-display text-text uppercase">
                                 ...
                                 <p class="text-xs heading-muted mt-1">Outliers (>2x): </p>`
        })}
                        ${Components.createCard({
            title: 'Fornecedores Ativos',
            content: `<p class="text-4xl font-display text-${stats.supplierDiversity?.status === 'good' ? 'primary' : stats.supplierDiversity?.status === 'warning' ? 'amber-500' : 'alert'} uppercase">
                                    ${stats.supplierDiversity?.totalFornecedores || 0}
                                 </p>
                                 <p class="text-sm heading-muted mt-1">
                                    Concentra‡Æo: ${(stats.supplierDiversity?.concentracao || 0).toFixed(1)}%
                                 </p>
                                 <p class="text-xs heading-muted mt-1">Top:  | Meta: < 30%</p>`
        })}
                    </div>
                </div>
